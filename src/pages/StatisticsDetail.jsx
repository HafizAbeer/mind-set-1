...
import { Share2, LayoutDashboard, FileText, BarChart3, BookOpen, RotateCcw, Gauge, Plus, Link, ChevronDown } from "lucide-react";
...
const StatCard = ({ icon, title, items, themeColor }) => (
  ...
  >
    <div className="flex items-center gap-3.5">
      ...
      />
      <h3 className="text-[20px] font-inter font-bold tracking-tight" style={{ color: themeColor }}>
        {title}
      </h3>
    </div>
...
const DiaryEntry = ({ timestamp, sentiment, motivation, text }) => (
  <div className="rounded-[24px] p-7 border border-[#4070DA]/60 bg-[#1A1C26]/40 backdrop-blur-md flex flex-col gap-5 transition-all hover:bg-[#1A1C26]/60">
    <div className="flex items-center justify-between">
      <span className="text-[16px] font-inter font-bold text-white/90">{timestamp}</span>
      <div
        className={`px-5 py-1.5 rounded-full text-[12px] font-inter font-bold border uppercase tracking-widest ${sentiment === "Positive"
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        <span className="text-[14px] font-inter font-bold text-[#4070DA] uppercase tracking-wider">Motivation:</span>
        <span className="text-[16px] font-inter font-medium text-white/95">{motivation}</span>
      </div>
      <div className="flex flex-col gap-1">
        <span className="text-[14px] font-inter font-bold text-[#4070DA] uppercase tracking-wider">Entry:</span>
        <p className="text-[16px] font-inter font-medium text-white/80 leading-relaxed font-italic line-clamp-3">{text}</p>
      </div>
    </div>
  </div>
);
...
const ReflectionEntry = ({ timestamp, combination, text }) => (
  <div className="rounded-[24px] p-7 border border-[#4070DA]/60 bg-[#1A1C26]/40 backdrop-blur-md flex flex-col gap-5 transition-all hover:bg-[#1A1C26]/60">
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        <span className="text-[14px] font-inter font-bold text-[#4070DA] uppercase tracking-wider">Combination:</span>
        <span className="text-[16px] font-inter font-medium text-white/95 leading-tight">{combination}</span>
      </div>
      <div className="flex flex-col gap-1">
        <span className="text-[14px] font-inter font-bold text-[#4070DA] uppercase tracking-wider">Entry:</span>
        <p className="text-[16px] font-inter font-medium text-white/80 leading-relaxed font-italic line-clamp-4">{text}</p>
      </div>
    </div>
  </div>
);
...
const MetricCard = ({ title, value, color }) => (
  <div className="rounded-[24px] p-6 border border-white/10 bg-[#1A1C26]/40 backdrop-blur-sm flex flex-col items-center justify-center gap-3 transition-all hover:bg-[#1A1C26]/60 min-h-[140px]">
    <span className="text-[14px] font-inter font-bold text-white/60 text-center">{title}</span>
    <span className="text-[44px] font-inter font-bold leading-none" style={{ color }}>{value}</span>
  </div>
);
...
const SatelliteMetric = ({ title, value, trend, trendColor }) => (
  <div className="w-full sm:w-auto rounded-[24px] p-6 border border-white/10 bg-[#1C1C24]/60 backdrop-blur-md flex flex-col gap-1.5 transition-all hover:scale-[1.05] hover:bg-[#1C1C24]/80 shadow-2xl min-w-0 sm:min-w-[220px]">
    <span className="text-[14px] font-inter font-bold text-white/60 uppercase tracking-widest">{title}</span>
    ...
    <span className="text-[12px] font-inter font-bold" style={{ color: trendColor }}>
      {trend} <span className="text-white/40 font-medium lowercase">since last month</span>
    </span>
  </div>
);
...
const StatisticsDetail = () => {
  ...
  const col1 = [
    { title: "Top Mindsets", icon: mindsetIcon, themeColor: "#FF595C", items: ["Joy", "Serenity", "Worry", "Anger"] },
    { title: "Causes", icon: causeIcon, themeColor: "#88EC65", items: ["Ambition", "Excessive Self-Criticism", "Self-Worth", "Feeling Powerful"] },
    { title: "Symptoms", icon: symptomIcon, themeColor: "#2AABEE", items: ["Pain", "Sweating", "tinnitus", "nausea"] },
    { title: "Exercises", icon: exerciseIcon, themeColor: "#FF6721", items: ["Yoga", "pet care", "meet friends", "tidying up"] }
  ];

  const col2 = [
    { title: "Trigger", icon: triggerIcon, themeColor: "#526FFF", items: ["Love", "Children", "Parents", "Weather"] },
    { title: "Body Structures", icon: bodyIcon, themeColor: "#D16868", items: ["Upper Spine", "Jaw", "Cervical", "Stomach"] },
    { title: "Intention", icon: intentionIcon, themeColor: "#FBA90B", items: ["Serenity", "Selfcare", "Empathy", "offensive spirit"] },
    { title: "Life script", icon: scriptIcon, themeColor: "#EEF05C", items: ["Serenity", "Selfcare", "Empathy", "offensive spirit"] }
  ];
  ...
  const protocolData = [
    {
      ...
      tags: [
        { label: "Serenity", color: "#FF595C" }, { label: "Parents", color: "#526FFF" }, { label: "Low self-worth", color: "#88EC65" }, { label: "Jaw", color: "#D16868" }, { label: "Inner restlessness", color: "#2AABEE" }, { label: "Disciplin", color: "#FBA90B" }, { label: "Yoga", color: "#FF6721" }, { label: "Smart", color: "#EEF05C" }, { label: "Week", color: "#88EC65" }, { label: "Obedient", color: "#7C3AED" }
      ]
    },
    {
      timestamp: "Nov 16, 2025, 02:37 PM",
      tags: [
        { label: "Serenity", color: "#FF595C" }, { label: "Parents", color: "#526FFF" }, { label: "Low self-worth", color: "#88EC65" }, { label: "Jaw", color: "#D16868" }, { label: "Inner restlessness", color: "#2AABEE" }, { label: "Disciplin", color: "#FBA90B" }, { label: "Yoga & Sports", color: "#FF6721" }, { label: "Smart", color: "#EEF05C" }, { label: "Week", color: "#88EC65" }, { label: "Obedient", color: "#7C3AED" }
      ]
    }
  ];
  ...
  const diaryData = [
    {
      timestamp: "Nov 16, 2025, 02:37 PM", sentiment: "Negative", motivation: "Self Growth", text: "We often judge ourselves harshly for our bad days."
    },
    {
      timestamp: "Nov 16, 2025, 02:37 PM", sentiment: "Positive", motivation: "Self Growth", text: "We often judge ourselves harshly for our bad days (the red spikes in your graph), viewing them as failures. But in reality, those days are just user feedback. They are data points telling you that a specific \"feature\" of your life—your sleep, your boundaries, or your routine—needs a patch or an update."
    }
  ];

  const reflectionData = [
    {
      timestamp: "Nov 16, 2025, 02:37 PM", combination: "Angry-work-impatience", text: "We often judge ourselves harshly for our bad days."
    },
    {
      timestamp: "Nov 16, 2025, 02:37 PM", combination: "Angry-work-impatience", text: "We often judge ourselves harshly for our bad days."
    }
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8 w-full md:w-auto">
              <div className="flex items-center gap-5">
                ...
                <div className="flex flex-col">
                  <h1 className="text-[28px] sm:text-[38px] font-inter font-bold leading-tight tracking-tight">Statistics</h1>
                  <p className="text-[16px] sm:text-[20px] font-inter font-medium text-[#9CA1A7]">Track your mindfulness journey</p>
                </div>
              </div>
                ...
                <div className="flex items-center bg-[#1C1C24] p-1 rounded-xl border border-white/5 sm:ml-4 w-full sm:w-auto overflow-x-auto custom-scrollbar">
                  {timeFilters.map(filter => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg text-[13px] font-inter font-bold transition-all whitespace-nowrap ${timeFilter === filter
            ...
            <nav className="hidden md:flex items-center bg-[#1A1C26] rounded-[12px] h-[44px] border-[1.5px] border-[#595C66] p-[4px] overflow-x-auto custom-scrollbar">
              {tabs.map(tab => (
                <button
                  ...
                  className={`flex-1 min-w-[140px] flex items-center justify-center gap-2 h-full rounded-[8px] text-[14px] font-inter font-bold transition-all whitespace-nowrap ${activeTab === tab.name
                    ? "bg-[#4070DA] text-white shadow-lg"
                  ...
                  <div className="max-h-[50dvh] overflow-auto custom-scrollbar">
                    {tabs.map((tab) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-2 rounded-[8px] px-3 py-2 text-left text-[12px] font-inter font-bold transition-all ${activeTab === tab.name
            ...
            {activeTab === "Overview" ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 animate-in fade-in duration-500">
                <div className="flex flex-col gap-8">
                  {col1.map((section, idx) => <StatCard key={idx} {...section} />)}
                </div>
                <div className="flex flex-col gap-8">
                  {col2.map((section, idx) => <StatCard key={idx} {...section} />)}
                </div>
              </div>
            ) : activeTab === "Protocol" ? (
              <div className="flex flex-col gap-8 max-w-[1300px] animate-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex flex-col gap-6">
                  {protocolData.map((entry, idx) => <ProtocolEntry key={idx} {...entry} />)}
                </div>
              </div>
            ) : activeTab === "Graph" ? (
              <div className="flex flex-col gap-10 max-w-[1300px] animate-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="rounded-[24px] p-8 border border-[#4070DA]/60 bg-[#1A1C26]/40 backdrop-blur-md flex flex-col gap-8">
                  <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-[24px] font-inter font-bold text-white/95">Stress Analysis (Overall)</h2>
                  </div>
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-6">
                    <MetricCard title="Positive Mindsets" value="7" color="#88EC65" />
                    <MetricCard title="Unclear Mindsets" value="9" color="#EEF05C" />
                    <MetricCard title="Negative Mindsets" value="2" color="#FF595C" />
                    <MetricCard title="Exercises Scores" value="4" color="#4070DA" />
                    <MetricCard title="Mantra Scores" value="3" color="#FBA90B" />
                    <MetricCard title="Anchor Scores" value="4" color="#FF595C" />
                  </div>
                  <div className="w-full lg:w-[320px]">
                    <div className="rounded-[24px] p-8 border border-[#4070DA]/60 bg-[#1A1C26]/60 backdrop-blur-md flex flex-col items-center justify-center gap-6 h-full lg:mt-20 max-h-[150px] text-center shadow-2xl">
                      ...
                      <span className="text-[72px] font-inter font-bold text-[#88EC65] leading-none drop-shadow-[0_0_20px_rgba(136,236,101,0.3)]">0.67</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 animate-in zoom-in-95 duration-700 justify-items-center items-start">
                ...
                <SatelliteMetric title="Positive Mindsets" value="42" trend="+31%" trendColor="#88EC65" />
                <SatelliteMetric title="Unclear Mindsets" value="13" trend="+11%" trendColor="#EEF05C" />
                <SatelliteMetric title="Negative Mindsets" value="17" trend="-24%" trendColor="#FF595C" />
                <SatelliteMetric title="Total Entries" value="47" trend="+12%" trendColor="#88EC65" />
                ...
                <div className="w-full md:col-span-2 lg:col-span-1 max-w-[540px] rounded-[32px] p-6 sm:p-10 border border-white/10 bg-[#1A1C30] backdrop-blur-3xl shadow-3xl flex flex-col gap-6 sm:gap-8 relative overflow-hidden group">
                  ...
                  <div className="relative w-full aspect-square flex items-center justify-center py-4 sm:py-6">
                    <svg viewBox="0 0 340 340" className="w-full h-full max-w-[340px] transform -rotate-90">
                      {/* Inner Circle Track */}
                      <circle cx="170" cy="170" r="115" fill="transparent" stroke="rgba(255,255,255,0.03)" strokeWidth="80" />
                      ...
                      <circle
                        ...
                        style={{ filter: 'drop-shadow(0 0 15px rgba(136,236,101,0.25))' }}
                      />
                  ...
                  <div className="flex flex-col gap-4 mt-2 px-6">
                    <div className="flex items-center justify-between group/item">
                      <div className="flex items-center gap-4">
                        <div className="w-2.5 h-2.5 rounded-full bg-[#88EC65] shadow-[0_0_8px_#88EC65]" />
                        <span className="text-[16px] font-inter font-bold text-white/80 group-hover/item:text-white transition-colors">Positive Mindset</span>
                      </div>
                      <span className="text-[16px] font-inter font-bold text-white/95">+12%</span>
                    </div>

                    <div className="flex items-center justify-between group/item">
                      <div className="flex items-center gap-4">
                        <div className="w-2.5 h-2.5 rounded-full bg-[#FF595C] shadow-[0_0_8px_#FF595C]" />
                        <span className="text-[16px] font-inter font-bold text-white/80 group-hover/item:text-white transition-colors">Negative Mindset</span>
                      </div>
                      <span className="text-[16px] font-inter font-bold text-white/95">-8%</span>
                    </div>

                    <div className="flex items-center justify-between group/item">
                      <div className="flex items-center gap-4">
                        <div className="w-2.5 h-2.5 rounded-full bg-[#EEF05C] shadow-[0_0_8px_#EEF05C]" />
                        <span className="text-[16px] font-inter font-bold text-white/80 group-hover/item:text-white transition-colors">Unclear Mindset</span>
                      </div>
                      <span className="text-[16px] font-inter font-bold text-white/95">30%</span>
                    </div>

                    <div className="flex items-center justify-between group/item">
                      <div className="flex items-center gap-4">
                        <div className="w-2.5 h-2.5 rounded-full bg-[#4070DA] shadow-[0_0_8px_#4070DA]" />
                        <span className="text-[16px] font-inter font-bold text-white/80 group-hover/item:text-white transition-colors">Exercises score</span>
                      </div>
                      <span className="text-[16px] font-inter font-bold text-white/95">12%</span>
                    </div>
                  </div>
                ...
                <SatelliteMetric title="Mind-o-Meter" value="36" trend="+12%" trendColor="#88EC65" />
                <SatelliteMetric title="Total Exercises" value="36" trend="+12%" trendColor="#88EC65" />
                <SatelliteMetric title="Total Mantras" value="36" trend="+12%" trendColor="#88EC65" />
                <SatelliteMetric title="Total Anchors" value="36" trend="+12%" trendColor="#88EC65" />
              </div>
            ) : activeTab === "Diary" ? (
              <div className="flex flex-col gap-8 max-w-[1300px] animate-in slide-in-from-bottom-4 duration-500">
                {!showNewDiaryEntry ? (
                  <>
                    ...
                    <div className="flex flex-col gap-6">
                      {diaryData.map((entry, idx) => <DiaryEntry key={idx} {...entry} />)}
                    </div>
                  </>
                ) : (
                  <div className="flex flex-col gap-8 animate-in fade-in duration-500">
                    ...
                    <div className="rounded-[24px] p-6 md:p-10 border border-[#4070DA]/30 bg-[#1A1C30]/60 backdrop-blur-xl shadow-2xl flex flex-col gap-10">
                      {/* Mood Selection */}
                      <div className="flex flex-col gap-4">
                        <label className="text-[18px] font-inter font-bold text-white/90">Current Mood</label>
                        <div className="flex flex-col gap-3">
                          {[
                            { label: "Positive", color: "#FF595C" }, // Reddish label in screenshot? Wait, let me re-check.
                            { label: "Uncertain", color: "#EEF05C" },
                            { label: "Negative", color: "#4070DA" } // Blue label in screenshot?
                          ].map((mood) => (
                            <label key={mood.label} className="flex items-center gap-3 cursor-pointer group w-fit">
                              <div className="relative w-5 h-5 flex items-center justify-center">
                                <input
                                  type="radio"
                                  name="mood"
                                  className="peer appearance-none w-5 h-5 rounded-full border-2 border-white/20 checked:border-[#4070DA] transition-all"
                                />
                      ...
                      <div className="flex flex-col gap-4">
                        ...
                        <input
                          ...
                          className="w-full bg-[#0d0d14]/40 border border-white/10 rounded-xl px-5 py-4 text-white font-inter placeholder:text-white/20 focus:outline-none focus:border-[#4070DA]/50 transition-all"
                        />
                      </div>
                      ...
                      <div className="flex flex-col gap-4">
                        ...
                        <textarea
                          ...
                          rows={8}
                          className="w-full bg-[#0d0d14]/40 border border-white/10 rounded-xl px-5 py-4 text-white font-inter placeholder:text-white/20 focus:outline-none focus:border-[#4070DA]/50 transition-all resize-none"
                        />
                      </div>
                ...
                {!showNewReflectionEntry ? (
                  <>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                      {reflectionData.map((entry, idx) => <ReflectionEntry key={idx} {...entry} />)}
                    </div>
                  </>
                ) : (
                  <div className="flex flex-col gap-8 animate-in fade-in duration-500">
                    <div className="flex flex-col gap-2 py-4">
                      ...
                      <p className="text-[18px] font-inter font-medium text-white/60">Dialectical thinking: Separate emotional from rational aspects</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                      ...
                      <div className="rounded-[32px] p-8 bg-[#4070DA] flex flex-col gap-8 shadow-2xl min-h-[500px]">
                        <h3 className="text-[22px] font-inter font-bold text-white leading-tight min-h-[64px]">
                          Emotional aspects; Pro aspects;<br />
                      ...
                      <div className="rounded-[32px] p-8 bg-[#4070DA] flex flex-col gap-8 shadow-2xl min-h-[500px]">
                        <h3 className="text-[22px] font-inter font-bold text-white leading-tight min-h-[64px]">
                          Rational aspects; contra aspects;<br />
